package net.manzj.demo.ucenter.client;


import net.manzj.demo.ucenter.util.XMLHelper;
...
public class Client extends PHPFunctions{
	...
	@Value("${ucenter.uc_ip}")
	public  String UC_IP;
	@Value("${ucenter.uc_api}")
	public  String UC_API;
	@Value("${ucenter.uc_connect}")
	public  String UC_CONNECT;
	@Value("${ucenter.uc_key}")
	public  String UC_KEY;
	@Value("${ucenter.uc_appid}")
	public  String UC_APPID;
	@Value("${ucenter.uc_client_release}")
	public  String UC_CLIENT_RELEASE;
	@Value("${ucenter.uc_api_func}")
	public  String UC_API_FUNC;
	...
	protected String uc_unserialize(String $s) {
		LinkedList<String> list = XMLHelper.uc_unserialize($s);
		if(list.size() > 0){
			return list.get(0);
		}else {
			return $s;
...
//		}
	...
	*/
	...
	public String uc_user_active(String $username, String $password, String $email){
		...
		String $postdata = uc_api_requestdata("", "", $s.toString(),""); 

		int len = UC_API.lastIndexOf('/');
		if(len<5) {return "-1";}
		String prefix = UC_API.substring(0,len);
		return uc_fopen2(prefix+"/api/active.php", 500000, $postdata, "", true, UC_IP, 20, true);
	}
}
